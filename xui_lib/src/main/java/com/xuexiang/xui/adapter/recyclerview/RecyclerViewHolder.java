...
public class RecyclerViewHolder extends RecyclerView.ViewHolder {
    ...
    public RecyclerViewHolder checked(@IdRes int id, boolean checked) {
        ...
        if (view instanceof SwitchButton) {
            ((SwitchButton) view).setChecked(checked);
        }
        return this;
    }

    //自己加的给滑块选择器设置值
    public RecyclerViewHolder setLevel(@IdRes int id, int level){
        View view = findView(id);
        if(view instanceof XSeekBar) {
            ((XSeekBar) view).setDefaultValue(level);
        }
        return this;
    }
    //监听滑块
    public RecyclerViewHolder setXSeekBarListener(@IdRes int id, XSeekBar.OnSeekBarListener listener){
        View v = findView(id);
        if (v instanceof XSeekBar){
            ((XSeekBar) v).setOnSeekBarListener(listener);
        }
        return this;
    }
